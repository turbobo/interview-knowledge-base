...
| [JVM 内存区域](./JVM内存区域.md) | 5 大内存区域 + 直接内存、分代结构、JDK 版本演进、OOM 速查表 | 中级 / 高级 |
| [JVM 内存区域全景图](./JVM内存区域全景图.html) | 可视化知识图（浏览器打开），含堆分代图、时间线、OOM 速查表 | 中级 / 高级 |
...
| [JVM 栈与堆：引用 vs 对象](./JVM栈与堆-引用vs对象.md) | 栈存引用/堆存对象、内存布局图、引用实现方式、String 存储位置 | 初级 / 中级 |
